...
function isRegionOfInterestObject (obj) {
  return typeof obj === 'object' &&
    obj.hasOwnProperty('minX') &&
    obj.hasOwnProperty('minY') &&
    obj.hasOwnProperty('maxX') &&
    obj.hasOwnProperty('maxY');
}
describe('ROISensor test', function() {
  before(function() {
    ctx = new rs2.Context();
    const sensors = ctx.querySensors();
    assert(sensors.length > 0); // Sensor must be connected
    for (let s of sensors) {
      if (s instanceof rs2.ROISensor) sensor = s;
    }
    assert(sensor !== undefined, 'ROISensor should exist');
  });
  ...
  it.only('Testing method getRegionOfInterest, return value', () => {
    let rtn;
    assert.doesNotThrow(() => {
      rtn = sensor.getRegionOfInterest();
    });
    const ROIProperties = {
      minX: 'number',
      minY: 'number',
    ...
    assert(isRegionOfInterestObject(rtn, ROIProperties));
console.log(Object.getOwnPropertyNames(rtn));
  });
